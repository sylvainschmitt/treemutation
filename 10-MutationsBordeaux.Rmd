  ...
  ggplot(aes(AF, col = dataset), fill = NA) +
    geom_label(aes(y = y, col = dataset, label = label),
             data.frame(study = rep(c("Schmid-Siegert et al., (2017)", "Plomion et al. (2018)"), each = 4),
                        ...
                        label = c("N=12", "N=510 000\n(709 000)", "N=223\n(310)", "N=87\n(121)",
                                  "N=60", "N=464\n(552)", "N=460\n(548)", "N=108\n(129)"),
                        AF = c(0.3, 0.4, 0.05, 0.2,
                               0.35, 0.15, 0.18, 0.2),
                        y = c(7.2, 3, 7, 7,
                              4.5, 10, 8, 5)) %>%
               mutate(dataset = factor(dataset, levels = c("Original studies", "GATK", "Mutect2",
...
# ggplot(data, aes(POS3f, POS5f, fill = P)) +
#   geom_tile() +
#   facet_grid(dataset ~ type) +
#   viridis::scale_fill_viridis("Percentage", direction = -1, option = "A") +
#   theme(legend.position = "bottom") + xlab("3'") + ylab("5'") +
