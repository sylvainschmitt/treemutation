...
```{r}
library(vcfR)
vcf <- read.vcfR("data/mutations/angela/trees/base.vcf.gz")
gt <- extract.gt(vcf, element="GT", as.numeric=TRUE, IDtoRowNames=FALSE)
heatmap.bp(gt[1:2598,])
```

```{r}
vcf <- vcfR::read.vcfR("data/mutations/angela/trees/base.vcf.gz")
extract.gt(vcf, element="GT", as.numeric=TRUE, IDtoRowNames=FALSE)[,-11] %>% 
  cor() %>% 
  as.data.frame() %>% 
  mutate_all(round, 2)
```
...
library(ape)
vcf <- vcfR::read.vcfR("data/mutations/angela/trees/base.vcf.gz")
gt <- vcfR::extract.gt(vcf, element="GT", as.numeric=TRUE, IDtoRowNames=FALSE)[,-11] 
m <- dist.gene(t(gt))
t <- nj(m)
plot(t)
```
