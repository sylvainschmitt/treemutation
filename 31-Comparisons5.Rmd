...
data_m <- group_by(data, tree, synonymy) %>% 
  summarise(mutation_AF = median(mutation_AF))
gd <- ggplot(data, aes(synonymy, mutation_AF, fill = synonymy)) +
  geom_violin() +
  ylab("Allelic fraction") +
  scale_fill_manual(name = "",
                    values = c("#FF7D7D", "#B3FFAE"), 
                    labels = c("non-synonymous", "synonymous")) +
  facet_wrap(~tree) +
  scale_y_log10(limits = c(NA, 0.20)) +
  geom_point(data = data_m, size = 3) +
  ggpubr::stat_compare_means(label = "p.format", method = "t.test",
                             bracket.size	= 1.2, tip.length	= 0,
                             label.y = log10(0.15), vjust = -0.5,
                             comparisons = list(c("NS", "S"))) +
  theme(axis.text.x = element_blank(), axis.ticks = element_blank(), 
        axis.title.x = element_blank(), axis.line.x = element_blank(),
          panel.spacing = unit(0, "lines"), legend.position = c(0.7, 0.3), 
