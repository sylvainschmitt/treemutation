...
make_plot <- function(name = "Angela") {
  ...
  g1.A <- ggplot(filter(mutations_hist_log, tree == name),
  ...
  g1 <- g1.A + 
    ...
    , xmin = 0.25, xmax = 0.52, ymin = log(5), ymax = log(50)) +
  ...
  g2 <- ggplot() +
    geom_vline(aes(xintercept = mutation_AF), alpha = 1, col = "#ffc000",
               data = filter(mutations, tree == name, transmitted == "no") %>% 
                 select(-SNV) %>% mutate(mutation_AF = round(mutation_AF, 3)) %>% unique()) +
    geom_vline(aes(xintercept = mutation_AF), alpha = 1, size = 1, col = "#cc0000",
               data = filter(mutations, tree == name, transmitted == "yes")) +
    xlab("Allelic fraction (bin of 0.02)") +
  ...
  g3 <- cowplot::plot_grid(g1, g2, 
                           nrow = 2, align = "v", 
                           rel_heights = c(5,1))
  cowplot::save_plot(paste0("~/Téléchargements/", name, "AF.png"), g3, dpi = 800)
}
# make_plot("Angela")
lapply(c("Angela", "Sixto", "Napoleon", "3P", "Verzy"), make_plot)
```
...
mutations_hist_log <- mutations %>%
  ...
  group_by(tree, mutation_AF_class) %>% 
  summarise(N = n())
g1.B <- ggplot(filter(mutations_hist, tree == "Angela"),
       aes(mutation_AF_class, N+10)) +
  geom_col(fill = "#ffc000", col = NA) +
  xlim(0, 0.52) +
  xlab("Allelic fraction (bin of 0.001)")
g1.A <- ggplot(filter(mutations_hist_log, tree == "Angela"),
       aes(mutation_AF_class, N+1)) +
  geom_col(fill = "#ffc000", col = NA) +
  ylab("Count") +
  xlim(0, 0.52)
g1 <- g1.A + 
  ...
  , xmin = 0.2, xmax = 0.5, ymin = log(2), ymax = log(50)) +
  theme(axis.title.x = element_blank(), axis.line.x = element_blank(),
        axis.ticks.x = element_blank(), axis.text.x = element_blank(),
        plot.margin = unit(c(0, 0, 0, 0), "cm"))
g2 <- ggplot() +
...
mutations_hist_log <- mutations %>%
  ...
  group_by(tree, mutation_AF_class) %>% 
  summarise(N = n())
g1.B <- ggplot(filter(mutations_hist, tree == "Sixto"),
       aes(mutation_AF_class, N+10)) +
  geom_col(fill = "#ffc000", col = NA) +
  xlim(0, 0.52) +
  xlab("Allelic fraction (bin of 0.001)")
g1.A <- ggplot(filter(mutations_hist_log, tree == "Sixto"),
       aes(mutation_AF_class, N+1)) +
  geom_col(fill = "#ffc000", col = NA) +
  ylab("Count") +
  xlim(0, 0.52)
...
make_plot_af <- function(name = "Angela") {
  ...
  g1 <- ggplot(filter(mutations_hist_log, tree == name),
                 aes(mutation_AF_class, N+1)) +
    geom_col(aes(fill = "crown"), col = NA) +
    geom_col(aes(fill = "tested"), col = NA, 
           data = filter(mutations_hist_log_tested, tree == name)) +
    geom_col(aes(fill = "transmitted"), col = NA, 
           data = filter(mutations_hist_log_embryo, tree == name)) +
    ylab("Count") + xlab("Allelic fraction (bin of 0.02)") +
    xlim(0, 0.52) +
    scale_fill_manual("", values = c("#ffc000", "grey", "#cc0000")) +
...
br <- 0.05
gc <- list("Napoleon (60X)" = vroom::vroom("data/mutations/swiss/leaf_mutations.tsv"),
     "3P (160X)" = vroom::vroom("data/mutations/bordeaux/leaf_mutations.tsv"),
     "Verzy (64X)" = vroom::vroom("data/mutations/hetre/leaf_mutations.tsv"),
    "Angela (138X)" = vroom::vroom("data/mutations/angela/filters/leaf_mutations.tsv"),
     "Sixto (73X)" = vroom::vroom("data/mutations/sixto/filters/leaf_mutations.tsv")) %>%
  bind_rows(.id = "tree") %>% 
  mutate(Filter = recode(Filter, "evs" = "evs base")) %>% 
  separate_rows(Filter) %>% 
  group_by(tree, Filter) %>% 
  ...
  mutate(tree = factor(tree, levels = c("Verzy (64X)", "Napoleon (60X)", "3P (160X)", "Sixto (73X)", "Angela (138X)"))) %>% 
  ggplot(aes(x = mutation_AF_class, m, col = tree, fill = tree)) +
  geom_ribbon(aes(ymin = l, ymax = h), col = NA, alpha = 0.5) +
  geom_line(aes(y = m)) +
  scale_y_log10(labels = scales::label_comma()) + 
  scale_color_manual("", values = cols) +
  xlab("Allelic fraction") + ylab("Cummulated mutations\nwith decreasing fraction") +
  theme(legend.position = c(0.7, 0.7), legend.text = element_text(size = 10)) +
...
data_inter <- list(Angela = read_tsv("save/mutations_angela_sixto.tsv"), 
  ...
  mutate(type = "All") %>% 
  mutate(category = ifelse(is.na(transcript), "inter.", "gene")) %>% 
...
data_syn <- list(Angela = read_tsv("save/mutations_angela_sixto.tsv"), 
  ...
  mutate(category = ifelse(is.na(nonsynonymous), "syn.", "non-syn.")) %>% 
  mutate(comparison = ifelse(is.na(nonsynonymous), "yes", "no")) %>% 
  dplyr::select(tree, SNV, type, category, comparison, mutation_AF)
data <- bind_rows(data_inter, data_intron, data_syn) %>% 
  filter(category %in% c("inter.", "intron", "syn.", "non-syn.")) %>% 
  mutate(comparison = factor(comparison, levels = c("yes", "no"))) %>% 
  mutate(category = factor(category, levels = c("inter.", "intron",
                                                "syn.", "non-syn."))) %>%
...
gd <- ggplot(data, aes(category, mutation_AF, fill = category)) +
  ...
  ggpubr::stat_compare_means(method = "t.test",  bracket.size	= 1.2,
                             label.y = c(log10(0.2), log10(0.3), log10(0.47)),
                             comparisons = list(c("non-syn.", "syn."), 
                                                c("non-syn.", "intron"),
                                                c("inter.", "non-syn."))) +
  scale_fill_manual(guide = "none", values  = c("#3581D8", "#28CC2D", "#FFE135", "#D82E3F")) +
  theme(axis.title.x = element_blank(), axis.text.x = element_text(size = 8), panel.spacing = unit(0.5, "lines")) +
  scale_y_log10(limits = c(NA, 0.6))
```

```{r}
g <- cowplot::plot_grid(ga, gb, gc, gd, nrow = 2, labels = c("a", "b", "c", "d"),
                        rel_heights = c(1, 1))
cowplot::save_plot(paste0("~/Téléchargements/Fig3.png"), g, dpi = 1000, base_width = 8, base_height = 6, bg = "white")
```
