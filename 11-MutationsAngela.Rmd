...
orfs %>% 
  ...
  ggtitle(paste("Angela:", n, "ORFs"), "Start M, Stop *, length >= 100")
rm(orfs, n)
```

```{bash, eval=F}
sqlite3 angela.annot.sqlite
sqlite> .headers on
sqlite> .mode csv
sqlite> .output angela.orf.csv
sqlite> SELECT * FROM ORF;
sqlite> .quit
```

```{r, circosAnnot, eval=F}
min_length <- 20*10^6
window <- 100*10^3
hs1_f <- filter(hs1, end > min_length)
orfs <- vroom::vroom("data/mutations/angela/annotation/angela.orf.csv") %>% 
  mutate(start = str_sub(peptide, 1, 1)) %>% 
  mutate(stop = str_sub(peptide, -1, -1)) %>% 
  mutate(length = rend - lend) %>% 
  filter(start == "M", stop == "*", length >= 100) %>% 
  separate(transcript_id, c("gene_id", "isoform"), remove = F)
gene_f <- vroom::vroom("data/mutations/angela/annotation/braker.gtf",
                   col_names = c("scf", "source", "type", "start", "end", 
                                 "score", "strand", "phase", "attributes")) %>% 
   filter(type  == "gene") %>% 
  filter(attributes %in% unique(orfs$gene_id)) %>% 
  mutate(name = gsub("Super-Scaffold_", "", scf)) %>% 
  filter(name %in% hs1_f$name) %>% 
  ...
  ungroup()
te_f <- read_tsv("data/mutations/angela/annotation/Dgu_HS1_HYBRID_SCAFFOLD.fa.out.gff", skip = 3, 
  ...
  mutate(interval = gsub("]", "", interval, fixed = T)) %>% 
  separate(interval, c("start", "stop"), ",", convert = T)
png("save/AngelaCircosAnnot.png", width = 1000, height = 1000, bg = NA)
circos.par(gap.degree = c(rep(1, nrow(hs1_f)-2), 5, rep(1, 1)),
           track.height = 0.2,
...
circos.yaxis(side = "left", at = c(200,400), labels.cex = 1)
dev.off()
circos.clear()
```

```{r}
include_graphics("save/AngelaCircosAnnot.png")
```
...
window <- 100*10^3
hs1_f <- filter(hs1, end > min_length)
mutation_f <- vroom::vroom("data/mutations/angela/filters/leaf_mutations.tsv") %>% 
...
transmitted_f <- readxl::read_xlsx("data/mutations/fruits/fruit_mutations_igv.xlsx", "classification") %>% 
  ...
  filter(tree == "Angela", transmitted == "yes") %>% 
...
```{r, eval=FALSE}
png("save/AngelaCircosmutations.png", width = 1000, height = 1000, bg = NA)
circos.par(gap.degree = c(rep(1, nrow(hs1_f)-2), 5, rep(1, 1)),
           track.height = 0.2,
           points.overflow.warning = F)
circos.initializeWithIdeogram(as.data.frame(hs1_f),
...
```{r}
include_graphics("save/AngelaCircosmutations.png")
```
...
mutation_f <- vroom::vroom("data/mutations/angela/filters/leaf_mutations.tsv") %>% 
  ...
  separate(interval, c("start", "stop"), ",", convert = T) %>% 
  ungroup()
gene_f <- read_tsv("data/mutations/angela/annotation/braker.gtf",
                   col_names = c("scf", "source", "type", "start", "end", "score", "strand", "phase", "attributes")) %>% 
  mutate(name = gsub("Super-Scaffold_", "", scf)) %>% 
  filter(name %in% hs1_f$name) %>% 
  filter(type  == "gene") %>% 
...
t2 <- mutation_f %>%   
  # left_join(gene_f) %>% 
  mutate(annot = paste0("A", 1:n())) %>% 
  mutate(name=paste0("Sc", name)) %>% 
    mutate(start = as.numeric(start+1),
         end = as.numeric(stop+1),
         mutations = as.numeric(mutations)) %>%
  dplyr::select(annot, name, start, end, mutations) %>% 
  as.data.frame()
chromoMap(list(t1), list(t2), data_based_color_map = T,
          data_type = "numeric", legend = T)
```
...
genes <- vroom::vroom("data/mutations/angela/annotation/braker.gtf",
                   col_names = c("scf", "source", "type", "start", "end", 
                                 "score", "strand", "phase", "attributes")) %>% 
   filter(type  == "gene") %>% 
  filter(attributes %in% unique(orfs$gene_id)) %>% 
...
ggplot(all, aes(percentage_gene, mutations)) +
  ...
  ggtitle("Angela", "Windows of 10^5")
...
windows <- hs1 %>% 
  ...
  mutate(window  = ivs::iv(window_start, window_end)) %>% 
  dplyr::select(name, window)
tes <- read_tsv("data/mutations/angela/annotation/Dgu_HS1_HYBRID_SCAFFOLD.fa.out.gff", skip = 3, 
               col_names = c("scf", "source", "type", "start", "end", "score", "strand", "phase", "attributes")) %>% 
  mutate(name = gsub("Super-Scaffold_", "", scf)) %>% 
  group_by(name) %>% 
...
write.vcf(tsv2vcf(mutations), "data/mutations/angela/trees/leaf_mutations.vcf.gz")
...
write.vcf(tsv2vcf(mutations), "data/mutations/angela/rev2//leaf_mutations.vcf.gz")
# wget https://raw.githubusercontent.com/edgardomortiz/vcf2phylip/master/vcf2phylip.py
# for file in $(ls *.vcf.gz); do python3 vcf2phylip.py -i $file -n ; done
files <- list.files(path = "data/mutations/angela/rev2/", pattern = ".nexus")
invisible(lapply(files, function(f){
  mutations <- phangorn::read.phyDat(file.path("data/mutations/angela/rev2", f), format = "nexus")
  mutations <- ape::as.DNAbin(mutations)
  ips::write.fas(mutations, file.path("data/mutations/angela/rev2", gsub("nexus", "fa", f)))
}))
```

```{r angleatipsphylo2, fig.width=12}
files <- list.files(path = "data/mutations/angela/trees/", pattern = ".min4.fa.treefile")
phylos <- lapply(files, function(f) read.tree(file.path("data/mutations/angela/trees", f)))
names(phylos) <- gsub(".min4.fa.treefile", "", files)
cowplot::plot_grid(plotlist = lapply(phylos, function(p) 
...
write_tsv(t, "data/mutations/angela/filters/leaf_mutations_spectra.tsv") 
```
...
```{r}
vroom::vroom("data/mutations/angela/filters/leaf_mutations_spectra.tsv") %>% 
...
fruit_mutations <- vroom::vroom("data/mutations/angela/filters/filtered_tip_mutations_spectra.tsv") %>% 
  ...
  mutate(rank = 1:n())    
# write_tsv(fruit_mutations, "data/mutations/angela/filters/angela_fruits_candidate_mutations.tsv")
```
...
rm(orfs)
DBI::dbDisconnect(annotation) ; rm(annotation)
write_tsv(t, "save/tmp.tsv")
read_tsv("save/tmp.tsv") %>% 
  ...
  write_tsv("data/mutations/angela/annotation/SNPsOnTrsc.tsv", col_names = F)
```

```{bash, eval=F, echo=T}
cd data/mutations/angela/annotation
...
read_tsv("save/tmp.tsv") %>% 
  ...
  left_join(read_tsv("data/mutations/angela/annotation/synonymy.output", col_names = T)) %>% 
...
```{r goeAngela}
go <- read_tsv("data/mutations/angela/annotation/trinotate_go_annotations.txt", col_names = c("gene", "id")) %>% 
  separate_rows(id, sep = ",") %>% 
  left_join(src_sqlite("data/mutations/angela/annotation/angela.annot.sqlite") %>% 
              tbl("go") %>% 
              collect())
genes <- read_tsv("save/tmp.tsv") %>% 
  mutate_at(c("lend", "rend"), as.numeric) %>% 
  mutate(posTrsc = POS - start + 1) %>% 
  filter(posTrsc >= lend, posTrsc <=   rend) %>% 
  mutate(`Chromosome Number` = toupper(attributes), `SNP Position` = posTrsc) %>% 
...
data.frame(type = c("All", "Within transcripts with genes", "Within CDS", "Non-synonymous"),
  ...
  coord_flip() +
  scale_y_log10() +
  ggtitle("Type of mutations for Angela") +
  xlab("") +
  geom_text(aes(y = 2, label = format(count, big.mark = " ")), col = "white")
```

```{r, fig.cap="Mutation allelic fraction per type."}
# genes <- genomeIntervals::readGff3("data/mutations/angela/annotation/trsc.fa.transdecoder.gff3")
# genes <- genomeIntervals::interval_union(genes)
# sum(genomeIntervals::width(genes))/sum(hs1$end)*100
data <- archi_mutations %>% 
  mutate(all = 1) %>% 
  left_join(read_tsv("save/tmp.tsv") %>% 
              dplyr::select(SNV) %>% 
              mutate(transcript = 1)) %>% 
  left_join(read_tsv("save/tmp.tsv") %>% 
              mutate_at(c("lend", "rend"), as.numeric) %>% 
...
g1 <- data %>% 
  ...
  ggtitle("All mutations AF > 0.01", "ratio:0.034 (<expected:0.139)") +
...
g2 <- filter(data, CDS == 1) %>% 
  ...
  ggpubr::stat_compare_means(method = "t.test") +
  ylab("Allelic fraction") +
  ggtitle("Mutations in CDS, AF > 0.01", "dN/dS=1.67") +
  geom_point(col = "red", size = 2,
...
data <- archi_mutations %>% 
  ...
  left_join(filter(goe@result, qvalue < 0.05) %>% 
              ...
              dplyr::select(SNV) %>% 
              mutate(goenriched = 1))
write_tsv(data, "save/mutations_angela_sixto.tsv")
filter(data, CDS == 1) %>% 
  filter(mutation_AF > 0.01) %>% 
  dplyr::select(SNV, mutation_AF, nonsynonymous) %>% 
  mutate(synonymy = ifelse(is.na(nonsynonymous), "S", "NS")) %>% 
...
goe@result %>% 
  ...
  ggtitle("GO enrichment for Angela non-synonymous mutations")
```
