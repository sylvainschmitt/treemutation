...
tip_mutations <- read_tsv("data/mutations/angela/tip_mutations.tsv") %>% 
  reshape2::dcast(tumor ~ SNV) %>% 
  mutate_if(is.numeric, funs(ifelse(is.na(.), 0, 1)))
stree = nj(dist.gene(tip_mutations))
stree$tip.label <- tip_mutations$tumor
