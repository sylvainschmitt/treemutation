...
gene_f <- read_tsv("data/mutations/sixto/annotation/braker.gtf",
                   col_names = c("scf", "source", "type", "start", "end", "score", "strand", "phase", "attributes")) %>% 
  mutate(name = gsub("Super-Scaffold_", "", scf)) %>% 
  filter(name %in% fai_f$name) %>% 
  filter(type  == "gene") %>% 
...
circos.yaxis(side = "left", at = c(20, 40, 60), labels.cex = 1)
...
circos.genomicTrack(gene_f, track.height = 0.1,
    panel.fun = function(region, value, ...) {
        circos.genomicLines(region, value, col = "#008000", border = "#008000", area = T, ...)
})
circos.yaxis(side = "left", at = c(20, 40, 60), labels.cex = 1)
...
circos.yaxis(side = "left", at = c(150, 30), labels.cex = 1)
