...
gene_f <- read_tsv("data/mutations/sixto/annotation/braker.gtf",
  ...
  separate(interval, c("start", "stop"), ",", convert = T) %>% 
  ungroup()
png("save/SixtoCircosAnnot.png", width = 1000, height = 1000)
circos.par(gap.degree = 4)
circos.initializeWithIdeogram(as.data.frame(fai_f))
circos.genomicTrack(gene_f,
    panel.fun = function(region, value, ...) {
        circos.genomicLines(region, value, col = "red", ...)
})
circos.yaxis(side = "left")
circos.genomicTrack(te_f, 
    panel.fun = function(region, value, ...) {
        circos.genomicLines(region, value, col = "blue", ...)
})
circos.yaxis(side = "left")
text(0, 0, "HS2", cex = 1.5)
legend("bottomleft", pch = 16, col = c("red", "blue"), legend = c("genes", "TE"), bty = "n")
dev.off()
circos.clear()
```

```{r circosAnnotSixto2, eval=F}
min_length <- 2*10^6
window <- 10*10^3
fai_f <- filter(fai, end > min_length)
te_f <- read_tsv("data/mutations/sixto/annotation/HS1_Sru_omap1_hap1_HYBRID_SCAFFOLD.fa.out.gff", skip = 3, 
...
gene_f <- read_tsv("data/mutations/sixto/annotation/braker.gtf",
  ...
  separate(interval, c("start", "stop"), ",", convert = T) %>% 
  ungroup()
annot <- left_join(te_f, gene_f) %>% 
  mutate(gene = ifelse(is.na(gene), 0, gene)) %>% 
  mutate(gene = log(gene + 1)) %>% 
  mutate(TE = log(TE + 1))
rm(te_f, gene_f)
te_breaks <- seq(0, max(annot$TE), by = 0.1)
col_te <- colorRamp2(te_breaks, viridis::viridis(length(te_breaks)))
gene_breaks <- seq(0, max(annot$gene), by = 0.1)
col_gene <- colorRamp2(gene_breaks, viridis::viridis(length(gene_breaks), option = "A"))
png("save/SixtoCircosAnnot.png", width = 1000, height = 1000)
circos.heatmap.initialize(as.vector(annot$TE), split = as.factor(annot$name))
circos.heatmap(as.vector(annot$TE), col = col_te, show.sector.labels = TRUE)
circos.heatmap(as.vector(annot$gene), col = col_gene)
circos.clear()
lgd_te <- Legend(title = "TE", col_fun = col_te, at =  seq(0, ceiling(max(annot$TE)), by = 1), labels = round(exp(seq(0, ceiling(max(annot$TE)), by = 1))+1))
lgd_gene <- Legend(title = "gene", col_fun = col_gene, at =  seq(0, ceiling(max(annot$gene)), by = 1), labels = round(exp(seq(0, ceiling(max(annot$gene)), by = 1))+1))
draw(packLegend(lgd_te, lgd_gene, direction = "horizontal"))
dev.off()
```
