...
# for file in $(ls *.min4.fa); do iqtree2 -s $file -redo -o R ; done
```

```{r}
vcf <- vcfR::read.vcfR("data/mutations/sixto/trees/base.vcf.gz")
vcf@fix %>% as_tibble() %>% 
  # group_by(CHROM) %>% 
  # summarise(N = n())
  filter(CHROM == "Super-Scaffold_10")
extract.gt(vcf, element="GT", as.numeric=TRUE, IDtoRowNames=FALSE)[,-9] %>% 
  cor() %>% 
  as.data.frame() %>% 
  mutate_all(round, 2)
```
...
```{r}
library(vcfR)
vcf <- read.vcfR("data/mutations/sixto/trees/base.vcf.gz")
gt <- extract.gt(vcf, element="GT", as.numeric=TRUE, IDtoRowNames=FALSE)
heatmap.bp(gt[1:236,])
```
